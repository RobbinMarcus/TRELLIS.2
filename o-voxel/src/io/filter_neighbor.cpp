...
torch::Tensor encode_sparse_voxel_octree_attr_neighbor_cpu(
    const torch::Tensor &coord, const uint32_t res, const torch::Tensor &attr) {
  ...
  for (int z = res - 1; z >= 0; z--) {
    for (int y = res - 1; y >= 0; y--) {
      for (int x = res - 1; x >= 0; x--) {
        ...
        if (x > 0 && buffer[tmp_ptr + C])
          neignbor_ptr = tmp_ptr;
        // y
        tmp_ptr = (z * res * res + (y - 1) * res + x) * (C + 1);
        if (y > 0 && buffer[tmp_ptr + C])
          neignbor_ptr = tmp_ptr;
        // z
        tmp_ptr = ((z - 1) * res * res + y * res + x) * (C + 1);
        if (z > 0 && buffer[tmp_ptr + C])
          neignbor_ptr = tmp_ptr;
        // xy
        tmp_ptr = (z * res * res + (y - 1) * res + (x - 1)) * (C + 1);
        if (y > 0 && x > 0 && buffer[tmp_ptr + C])
          neignbor_ptr = tmp_ptr;
        // xz
        tmp_ptr = ((z - 1) * res * res + y * res + (x - 1)) * (C + 1);
        if (z > 0 && x > 0 && buffer[tmp_ptr + C])
          neignbor_ptr = tmp_ptr;
        // yz
        tmp_ptr = ((z - 1) * res * res + (y - 1) * res + x) * (C + 1);
        if (z > 0 && y > 0 && buffer[tmp_ptr + C])
          neignbor_ptr = tmp_ptr;
        // xyz
        tmp_ptr = ((z - 1) * res * res + (y - 1) * res + (x - 1)) * (C + 1);
        if (z > 0 && y > 0 && x > 0 && buffer[tmp_ptr + C])
          neignbor_ptr = tmp_ptr;
        if (neignbor_ptr >= 0) {
          for (int c = 0; c < C; c++) {
  ...
  torch::Tensor delta =
      torch::zeros({static_cast<int64_t>(N), static_cast<int64_t>(C)},
 ...
 */
torch::Tensor
decode_sparse_voxel_octree_attr_neighbor_cpu(const torch::Tensor &coord,
                                             const uint32_t res,
