 ...
 */
torch::Tensor encode_sparse_voxel_octree_cpu(const torch::Tensor &codes,
  ...
  torch::Tensor svo_tensor =
      torch::from_blob(svo.data(), {static_cast<int64_t>(svo.size())},
                       torch::kUInt8)
          .clone();
  return svo_tensor;
}

void decode_sparse_voxel_octree_cpu_recursive(const uint8_t *svo,
                                              const uint32_t depth,
                                              uint32_t &ptr,
 ...
 */
torch::Tensor decode_sparse_voxel_octree_cpu(const torch::Tensor &octree,
                                             const uint32_t depth) {
  ...
  decode_sparse_voxel_octree_cpu_recursive(octree_data, depth, ptr, stack,
                                           codes);
  // Convert codes to tensor
  torch::Tensor codes_tensor =
      torch::from_blob(codes.data(), {static_cast<int64_t>(codes.size())},
                       torch::kInt32)
          .clone();
  return codes_tensor;
}
