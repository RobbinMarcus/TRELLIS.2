...
static inline U lerp(const T &a, const T &b, const T &t, const U &val_a,
                     const U &val_b) {
  if (a == b)
...
static auto get_or_default(const Map &map, const Key &key,
...
void intersect_qef(
    const Eigen::Vector3f &voxel_size, const Eigen::Vector3i &grid_min,
    ...
    std::vector<bool3>
        &intersected, // Output: Whether edge of voxel intersects with triangle
  ...
  for (size_t i = 0; i < N_tri; ++i) {
    ...
    // Scan-line algorithm to find intersections with the voxel grid from three
    // directions
    /*
      t0
      | \
    ...
    auto scan_line_fill = [&](const int ax2) {
      ...
      // Scan-line algorithm
      int start = std::clamp(int(t[0].y() / voxel_size[ax1]), grid_min[ax1],
                             grid_max[ax1] - 1);
      int mid = std::clamp(int(t[1].y() / voxel_size[ax1]), grid_min[ax1],
                           grid_max[ax1] - 1);
      int end = std::clamp(int(t[2].y() / voxel_size[ax1]), grid_min[ax1],
                           grid_max[ax1] - 1);

      auto scan_line_half =
          [&](const int row_start, const int row_end, const Eigen::Vector3d t0,
            ...
            for (int y_idx = row_start; y_idx < row_end; ++y_idx) {
              ...
              Eigen::Vector2d t3 =
                  lerp(t0.y(), t1.y(), y, Eigen::Vector2d(t0.x(), t0.z()),
                       Eigen::Vector2d(t1.x(), t1.z()));
              Eigen::Vector2d t4 =
                  lerp(t0.y(), t2.y(), y, Eigen::Vector2d(t0.x(), t0.z()),
                       Eigen::Vector2d(t2.x(), t2.z()));
              if (t3.x() > t4.x())
                std::swap(t3, t4);
              int line_start = std::clamp(int(t3.x() / voxel_size[ax0]),
                                          grid_min[ax0], grid_max[ax0] - 1);
              int line_end = std::clamp(int(t4.x() / voxel_size[ax0]),
                                        grid_min[ax0], grid_max[ax0] - 1);
...
void face_qef(
  ...
  for (size_t i = 0; i < N_tri; ++i) {
    ...
    float d_yz_e0 = -n_yz_e0.dot(Eigen::Vector2f(v0.y(), v0.z())) +
                    n_yz_e0.cwiseMax(0.0f).dot(
                        Eigen::Vector2f(voxel_size.y(), voxel_size.z()));
    float d_yz_e1 = -n_yz_e1.dot(Eigen::Vector2f(v1.y(), v1.z())) +
                    n_yz_e1.cwiseMax(0.0f).dot(
                        Eigen::Vector2f(voxel_size.y(), voxel_size.z()));
    float d_yz_e2 = -n_yz_e2.dot(Eigen::Vector2f(v2.y(), v2.z())) +
                    n_yz_e2.cwiseMax(0.0f).dot(
                        Eigen::Vector2f(voxel_size.y(), voxel_size.z()));
    ...
    float d_zx_e0 = -n_zx_e0.dot(Eigen::Vector2f(v0.z(), v0.x())) +
                    n_zx_e0.cwiseMax(0.0f).dot(
                        Eigen::Vector2f(voxel_size.z(), voxel_size.x()));
    float d_zx_e1 = -n_zx_e1.dot(Eigen::Vector2f(v1.z(), v1.x())) +
                    n_zx_e1.cwiseMax(0.0f).dot(
                        Eigen::Vector2f(voxel_size.z(), voxel_size.x()));
    float d_zx_e2 = -n_zx_e2.dot(Eigen::Vector2f(v2.z(), v2.x())) +
                    n_zx_e2.cwiseMax(0.0f).dot(
                        Eigen::Vector2f(voxel_size.z(), voxel_size.x()));
...
void boundry_qef(
  ...
  for (size_t i = 0; i < boundries.size() / 2; ++i) {
    ...
    Eigen::Matrix3f A =
        Eigen::Matrix3f::Identity() - (dir * dir.transpose()).cast<float>();
    ...
    Eigen::Vector3i step =
        (dir.array() > 0)
            .select(Eigen::Vector3i(1, 1, 1), Eigen::Vector3i(-1, -1, -1));
    ...
    for (int axis = 0; axis < 3; ++axis) {
      if (dir[axis] == 0.0) {
        ...
        float voxel_border =
            voxel_size[axis] * (v0_voxel[axis] + (step[axis] > 0 ? 1 : 0));
...
void face_from_dual_vertices(
    ...
    const std::vector<int3> &voxels, const std::vector<float3> &dual_vertices,
    const std::vector<bool3> &intersected, std::vector<int3> &face_indices) {
  for (int i = 0; i < dual_vertices.size(); ++i) {
    ...
    size_t neigh_indices[6] = {
        get_or_default(hash_table, VoxelCoord{coord.x + 1, coord.y, coord.z},
                       kInvalidIndex),
        get_or_default(hash_table, VoxelCoord{coord.x, coord.y + 1, coord.z},
                       kInvalidIndex),
        get_or_default(hash_table,
                       VoxelCoord{coord.x + 1, coord.y + 1, coord.z},
                       kInvalidIndex),
        get_or_default(hash_table, VoxelCoord{coord.x, coord.y, coord.z + 1},
                       kInvalidIndex),
        get_or_default(hash_table,
 ...
 * @param regularization_weight: Regularization factor to apply to the QEF
 * matrices.
...
mesh_to_flexible_dual_grid_cpu(const torch::Tensor &vertices,
                               ...
                               float face_weight, float boundary_weight,
                               float regularization_weight, bool timing) {
  ...
  std::vector<bool3>
      intersected; // Indicate whether edges of voxels intersect with surface
  std::vector<Eigen::Matrix4f> qefs; // QEF matrices for each voxel

  // Convert tensors to Eigen types
  Eigen::Vector3f e_voxel_size(voxel_size_ptr[0], voxel_size_ptr[1],
                               voxel_size_ptr[2]);
  Eigen::Vector3i e_grid_min(grid_range_ptr[0], grid_range_ptr[1],
                             grid_range_ptr[2]);
  Eigen::Vector3i e_grid_max(grid_range_ptr[3], grid_range_ptr[4],
                             grid_range_ptr[5]);
  ...
  for (int f = 0; f < F; ++f) {
    for (int v = 0; v < 3; ++v) {
      triangles.push_back(Eigen::Vector3f(v_ptr[f_ptr[f * 3 + v] * 3 + 0],
  ...
  if (timing)
    std::cout << "Intersect QEF computation took "
  ...
  if (face_weight > 0.0f) {
    ...
    if (timing)
      std::cout << "Face QEF computation took "
                << double(end - start) / CLOCKS_PER_SEC << " seconds."
                << std::endl;
  }
  ...
  if (boundary_weight > 0.0f) {
    ...
    for (const auto &e : edge_count) {
      if (e.second == 1) {
        ...
        boundries.push_back(Eigen::Vector3f(
            v_ptr[v0 * 3 + 0], v_ptr[v0 * 3 + 1], v_ptr[v0 * 3 + 2]));
        boundries.push_back(Eigen::Vector3f(
            v_ptr[v1 * 3 + 0], v_ptr[v1 * 3 + 1], v_ptr[v1 * 3 + 2]));
      }
    }
    ...
    if (timing)
      std::cout << "Boundary QEF computation took "
                << double(end - start) / CLOCKS_PER_SEC << " seconds."
                << std::endl;
  }
  ...
  for (int i = 0; i < voxels.size(); ++i) {
    ...
    if (regularization_weight > 0.0f) {
      ...
      Q += regularization_weight * cnt[i] *
           Qreg; // Scale by regularization weight
    }
    ...
    if (!(v_new.x() >= min_corner[0] && v_new.x() <= max_corner[0] &&
      ...
      auto solve_single_constraint = [&](int fixed_axis) {
        ...
        A << Q(ax1, ax1), Q(ax1, ax2), Q(ax2, ax1), Q(ax2, ax2);
        B << Q(ax1, fixed_axis), Q(ax1, 3), Q(ax2, fixed_axis), Q(ax2, 3);
  ...
  if (timing)
    std::cout << "Dual vertices computation took "
              << double(end - start) / CLOCKS_PER_SEC << " seconds."
              << std::endl;

  return std::make_tuple(
      torch::from_blob(voxels.data(), {int(voxels.size()), 3}, torch::kInt32)
          .clone(),
      torch::from_blob(dual_vertices.data(), {int(dual_vertices.size()), 3},
                       torch::kFloat32)
          .clone(),
      torch::from_blob(intersected.data(), {int(intersected.size()), 3},
                       torch::kBool)
          .clone());
}
